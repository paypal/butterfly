...
public class TransformationUtilityLoop extends TransformationUtility<TransformationUtilityLoop> implements TransformationUtilityParent {
    ...
    protected TransformationUtilityLoop setName(String name) {
        super.setName(name);
        if (template != null) template.setName(String.format(TEMPLATE_NAME_FORMAT, getName(), template.getSimpleClassName()));
        if (condition != null) condition.setName(String.format(CONDITION_NAME_FORMAT, getName(), condition.getSimpleClassName()));
        return this;
    }
